...
(1) 主 module 下新建 libs 文件夹，将：
`alipaysdk-noutdid-15.8.10.220419171600.aar` 以及
`missevansdk_Game_SDK_release_v2.0.6.aar` 文件放置到 libs 文件夹下
...
1. 由于加入微信支付功能，接入时，需要在自己的包名下新建 wxapi 文件夹，然后在此文件夹下新建 WXPayEntryActivity，并且 WXPayEntryActivity 继承 SDK 中的
...
获取 MissEvanSdk 对象后，可以调用其中接口。每个接口方法的参数中都包含 CallbackListener 类型的监听器，用户需实现其中的3个方法：onSuccess、onFailed 和
